...
在起始点前 700 ~ 200周期 处，之前音调的音量已经基本消失，并基本保持不变。我们把这一段称为 **谷段**，如图所示：

![valley](valley_part.png)

在起始点前 200周期 到起始点处，新的音调音量迅速爬升，然后取得一个 **峰值**。我们把这一段称为 **坡段**，如图所示：
...
结果如下图所示。其中中图为未去除 0 ~ 200周期 坡段的结果，下图为去除后的结果。可以看到，去除坡段确实提升了匹配精度。
...
* 比值过小的点（这里选取阈值为 3.5）
* 峰值过小的点（这里选取阈值为 0.1）
* 坡段上升速度过小的点（这里选取 50 ~ 0周期 与 100 ~ 50周期 的比值，阈值取 1.3）
...
```matlab
save guitar baseband band_wights tone
```
...
开始进行本次实验时，主要的一个困难在于对 MATLAB 不太了解。但随着实验的进行，这个困难也慢慢消失掉了 QAQ

从代码量上就可以看出来，本次实验最难的两个部分是 **节拍划分** 和 **基频判断**

对于节拍划分，开始时我参考了韩神的思路，尝试着求出信号的包络。然而可能是由于信号过于杂乱，利用 `hilbert` 进行希尔伯特变换后效果极差 TAT…而尝试了韩神的包络函数之后，却发现并没有使问题简单太多，故放弃包络的思路 = =

于是乎，我开始转而寻找在时域波形上寻找特点，最后决定利用其每个音调开始的快速增长段来定位起始点。中途为了精度又扣去了”坡段“，并加入了很多阈值和参数。好在最后的效果倒是十分的好，基本上能准确识别出所有的起始点。

对于基频判断，我开始时的思路只是简单地取频域的最大值。然而我很快注意到，真实的频谱中很容易出现谐波分量比基波分量要强的情况。于是才实现了在 1/2, 1/3, 1/4 倍频率附件寻找真正基频的算法。

不过需要注意的是，`fmt.wav` 中的音乐有很多个地方有和弦的存在，这直接导致有几个音调十分难判断，甚至导致误判。这是一个可以改进的地方。
